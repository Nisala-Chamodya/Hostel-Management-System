...
public class Validator {
    ...
    private static final Pattern typePattern = Pattern.compile("^[A-z /-]{2,20}$");
    private static final Pattern studentIdPattern=Pattern.compile("S-\\d{3,4}");
    private static final Pattern roomIdPattern=Pattern.compile("RM-\\d{3,4}");
    private static final Pattern custNicPattern=Pattern.compile("^([0-9]{9}[x|X|v|V]|[0-9]{12})$");
    private static final Pattern keyMoneyPattern=Pattern.compile("[0-9]+.[0-9]{2}");
    ...
    private static final Pattern qtyPattern=Pattern.compile("[0-9]+");
    ...
    public static boolean isRoomIdMatcher(String text){
        Matcher idMatcher=roomIdPattern.matcher(text);
        return  idMatcher.matches();
    }
    public static boolean isTypeMatcher(String text){
        Matcher idMatcher=typePattern.matcher(text);
        return  idMatcher.matches();
    }
    ...
    public static boolean isKeyMoneyMatcher(String text){
        Matcher isPriceMatcher=keyMoneyPattern.matcher(text);
    ...
    public static boolean isQtyMatcher(String text){
        Matcher isAvailabilityMatcher=qtyPattern.matcher(text);
